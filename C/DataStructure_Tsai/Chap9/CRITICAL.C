...
HeadNode *adjlist1[MAX_V+1];    /*相鄰串列*/
HeadNode *adjlist2[MAX_V+1];    /*反相鄰串列*/
...
void Toplogical_sort(HeadNode *adjlist[],int ES_LC[])
{
	...
	for ( vi = 1; vi <= N; vi++ )
		if ( adjlist[vi]->count == 0 )
			Push(&Stack1,vi);
	print_steps(ES_LC,0);   /*列出堆疊及ES_LC狀況*/
	for ( k=1; k<= N; k++ )
	{
		if (  Stack1.top == empty )
		{
