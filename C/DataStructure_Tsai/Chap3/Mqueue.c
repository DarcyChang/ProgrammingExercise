...
void enqueue_f(int number1)
{
	...
	else
	{
		...
		printf(" You insert %s to queue%d", MQ[r[number1-1]], number1 );
	}
}

void dequeue_f(int number2)
{
	if(r[number2-1] == f[number2-1]-1)  /* 當堆疊沒有資料存在，則顯示錯誤 */
		printf(" No item, queue%d is empty !",number2);
	else{

		printf(" Queue%d Item %s deleted", number2, MQ[f[number2-1]]);
		f[number2-1]++;
	}
}
...
void list_f(int number3)
{
	...
	if( (r[number3-1]) == f[number3-1]-1)
		printf(" No item, queue%d is empty!", number3);
	else{
